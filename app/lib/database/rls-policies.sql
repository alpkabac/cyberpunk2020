...
-- ---------------------------------------------------------------------------
-- Claim an unclaimed player character (user_id NULL) for the current user.
-- ---------------------------------------------------------------------------
...
-- GM may edit unclaimed PCs (user_id NULL).
...
-- Users can delete their own characters; any session participant may delete NPCs; GM may delete unclaimed PCs
CREATE POLICY "Users can delete their own characters"
  ON characters FOR DELETE
  USING (
    user_id = auth.uid() OR
    (type = 'npc' AND is_user_in_session(session_id)) OR
    (
      type = 'character' AND user_id IS NULL AND
      EXISTS (SELECT 1 FROM sessions WHERE id = session_id AND created_by = auth.uid())
    )
